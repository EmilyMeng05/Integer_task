"""Plan and run the fixed six-model T16 integer training matrix."""
...
from .training_t16 import automatic_device, train_t16_run


DEFAULT_CONFIG = Path("configs/henry_integer_t16_training.toml")
EXPECTED_SUBSET_SIZES = (16,)
...
def validate_protocol(config: Mapping[str, Any]) -> None:
    if tuple(config.get("task_order", ())) != TASK_NAMES:
        raise ValueError("task_order must match the verified T16 corpus")
    if tuple(config.get("task_subset_sizes", ())) != EXPECTED_SUBSET_SIZES:
        raise ValueError("task_subset_sizes must contain only 16")
    ...
    if int(config.get("total_records", -1)) != 8_000_000:
        raise ValueError("T16 corpus must contain 8,000,000 records")
    if int(config.get("records_per_task", -1)) != 500_000:
        raise ValueError("each T16 task must contain 500,000 records")
    if config.get("dataset_manifest") != "data/integer-8m-t16-v1/manifest.json":
        raise ValueError("dataset_manifest must identify the verified T16 corpus")
    ...
    for key, expected in frozen_training.items():
        if training.get(key) != expected:
            raise ValueError(f"training.{key} must remain {expected!r}")
    if data.get("max_sequence_length") != 1024:
        raise ValueError("data.max_sequence_length must remain 1024")
    if data.get("shuffle_buffer") != 10000:
        raise ValueError("data.shuffle_buffer must remain 10000")
...
def build_matrix(config_path: Path = DEFAULT_CONFIG) -> tuple[ProductionRun, ...]:
    ...
    if len(runs) != 6 or len({run.run_id for run in runs}) != 6:
        raise RuntimeError("T16 matrix must contain six unique runs")
    return tuple(runs)
...
def completion_is_valid(
    run: ProductionRun, *, config_path: Path = DEFAULT_CONFIG
) -> bool:
    try:
        config, config_sha256 = read_config(config_path)
        ...
        return bool(
            ...
            and set(validation) == set(run.tasks)
        )
...
def run_matrix(
    *,
    ...
    for run in runs:
        ...
        summary = train_t16_run(train_config)
        print(json.dumps(summary, indent=2, sort_keys=True), flush=True)
        expected_steps = preflight_steps or int(
            read_config(config_path)[0]["training"]["max_steps"]
        )
