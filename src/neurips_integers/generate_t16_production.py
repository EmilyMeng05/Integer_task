"""Resumable parallel generation of the eight-million-record T16 corpus."""
...
DEFAULT_COUNT = 8_000_000
DEFAULT_SHARD_SIZE = 80_000
DEFAULT_OUTPUT_DIR = Path("data/integer-8m-t16-v1")
...
def _write_shard(job: ShardJob) -> dict[str, Any]:
    ...
    with partial_path.open("wb") as raw:
        with gzip.GzipFile(
            filename="", mode="wb", compresslevel=GZIP_COMPRESSLEVEL,
            fileobj=raw, mtime=0,
        ) as compressed:
            for record_id in range(job.first_id, job.first_id + job.record_count):
                record = build_record(
                    ...
                    legacy_seed=job.legacy_seed,
                    new_seed=job.new_seed,
...
def _compatible_entry(job: ShardJob, entry: object) -> dict[str, Any] | None:
    ...
    if not isinstance(counts, Mapping) or set(counts) != set(TASK_NAMES):
        return None
    if sum(int(value) for value in counts.values()) != job.record_count:
        return None
    return dict(entry)


def _base_manifest(
    *, count: int, shard_size: int, min_digits: int, max_digits: int,
    legacy_seed: int, new_seed: int,
) -> dict[str, Any]:
...
def _assert_existing_manifest_compatible(
    ...
    keys = (
        "schema_version", "count", "base", "min_sampling_bucket",
        "max_sampling_bucket", "legacy_sampling_seed", "new_sampling_seed",
        "task_order_seed", "tasks", "shard_size", "shard_count", "splits",
    )
    mismatches = [key for key in keys if existing.get(key) != expected.get(key)]
    if mismatches:
        raise ValueError(
            "existing T16 manifest uses a different configuration: "
            + ", ".join(mismatches)
        )
...
def generate_production(
    ...
    *,
    ...
    if count // shard_size != 100:
        raise ValueError("T16 production data must contain exactly 100 shards")
    if workers < 1:
    ...
    manifest = _base_manifest(
        count=count, shard_size=shard_size, min_digits=min_digits,
        max_digits=max_digits, legacy_seed=legacy_seed, new_seed=new_seed,
    )
    ...
    if entries:
        print(f"Reusing {len(entries)}/100 verified T16 shards")
    ...
    def publish_progress() -> None:
        ...
        for entry in ordered:
            ...
            for task, counts in entry["sampling_stratum_counts"].items():
                stratum_counts[task].update(counts)
        _atomic_json({
            **manifest,
        ...
        with ProcessPoolExecutor(max_workers=min(workers, len(pending))) as pool:
            ...
            for completed, future in enumerate(as_completed(future_jobs), start=1):
                ...
                if completed == 1 or completed % 5 == 0 or completed == len(pending):
                    print(
                        f"Generated {completed}/{len(pending)} pending T16 shards "
                        f"({len(entries)}/100 total complete)", flush=True,
                    )
    else:
        publish_progress()
    ...
    if final.get("completed_shards") != 100:
        raise RuntimeError("T16 generation ended with incomplete shards")
    expected = count // len(TASK_NAMES)
    if final.get("task_counts") != {task: expected for task in TASK_NAMES}:
        raise RuntimeError("T16 task allocation is not exactly balanced")
    return final
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    result = generate_production(
        args.output_dir, count=args.count, shard_size=args.shard_size,
        min_digits=args.min_digits, max_digits=args.max_digits,
        legacy_seed=args.legacy_seed, new_seed=args.new_seed,
        workers=args.workers,
    )
    print(f"T16 production generation complete: {result['count']:,} records")
    print(f"Records per task: {result['records_per_task']:,}")
    print(f"Saved to: {args.output_dir}")
    print("Run verify_t16 before training.")
    return 0
